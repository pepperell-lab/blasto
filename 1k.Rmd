...
names(TG) <- c("CHROM", "POS", "ID", "REF", "ALT", "QUAL", "FILTER", "INFO", "FORMAT", "BDH001", "BDH002", "BDH003", "BDH004", "BDH005", "BDH006", "GTEx", "CANDREG", "CANDGENE")
...
Let's look at the variants in terms of 'rareness' to EUR and whether or not they are demonstrated eQTLs.
...
Of the `r length(same$POS)` SNPs that are fixed in the Hmong ROH consensus regions, `r sum(same$REG, na.rm=TRUE)` are annotated as regulatory in the 1K genomes data. Note that this method will not identify novel regulatory variants, as truly novel variants will not be in the 1K genomes data. `r sum(same_rare$REG, na.rm=TRUE)` of these regulatory SNPs are 'rare in EUR (< 10%), common in Hmong' and `r sum(same_rare_20$REG, na.rm=TRUE)` when loosening the rare to < 20 %. 
...
# Loose promising
prom3 <- filter(same_rare_20, REG == TRUE | (is.na(REG) & (!is.na(GTEx) | ImmVar == TRUE | !is.na(CANDREG) | !is.na(CANDGENE))))
```
