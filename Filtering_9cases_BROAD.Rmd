---
title: "Re-evaluating pilot Hmong WGS data with updated 1K genomes annotations and BROAD calling"
author: "Mary O'Neill"
date: "June 22 2017"
...
names(TG) <- c("CHROM", "POS", "ID", "REF", "ALT", "QUAL", "FILTER", "INFO", "FORMAT", "BDH001", "BDH002", "BDH003", "BDH004", "BDH005", "BDH006", "BDH007", "BDH008", "BDH009", "BDH010", "BDH018", "AF","EUR_AF","EAS_AF","SAS_AF","AFR_AF","AMR_AF","AA","TRI","CSQ","FUNSEQ","GTEx", "CANDREG", "CANDGENE")
...
AFgt <- TG.l %>%
  rowwise() %>%
  mutate(gt = gsub("/", "", paste(unlist(strsplit(BDH001, ":", fixed=TRUE))[1], 
                                  unlist(strsplit(BDH002, ":", fixed=TRUE))[1], 
                                  unlist(strsplit(BDH003, ":", fixed=TRUE))[1], 
                                  unlist(strsplit(BDH004, ":", fixed=TRUE))[1], 
                                  unlist(strsplit(BDH005, ":", fixed=TRUE))[1], 
                                  unlist(strsplit(BDH007, ":", fixed=TRUE))[1], 
                                  unlist(strsplit(BDH008, ":", fixed=TRUE))[1], 
                                  unlist(strsplit(BDH009, ":", fixed=TRUE))[1], 
                                  unlist(strsplit(BDH010, ":", fixed=TRUE))[1], 
                                  unlist(strsplit(BDH006, ":", fixed=TRUE))[1], 
                                  unlist(strsplit(BDH018, ":", fixed=TRUE))[1], sep="")))
...
same <- filter(AFgt, (substr(gt, 1, 2) == substr(gt, 3,4) | substr(gt, 1, 2) == substr(gt, 4,3)) & 
                      ...
                      (substr(gt, 1, 2) == substr(gt, 11,12) | substr(gt, 1, 2) == substr(gt, 12,11)) &
                      (substr(gt, 1, 2) == substr(gt, 13,14) | substr(gt, 1, 2) == substr(gt, 14,13)) &
                      (substr(gt, 1, 2) == substr(gt, 15,16) | substr(gt, 1, 2) == substr(gt, 16,15)) &
                      (substr(gt, 1, 2) == substr(gt, 17,18) | substr(gt, 1, 2) == substr(gt, 18,17))
                 )
...
#require("xlsx")
#write.table(prom4, "C://Users/Mary/PepLab/data/blasto/postdoc/170622_promising.txt", sep = "\t", eol = "\n", na = "NA", dec = ".", quote = FALSE, col.names=TRUE, row.names=FALSE, qmethod = "double")
```
